...
class Roles extends CatchModel
{
    ...
    public function getList()
    {
        ...
        $res = $this->catchSearch()
                    ->withoutField(['data_range','updated_at','deleted_at'])
                    ->with(['permissions', 'departments','users'])
                    ->order('id', 'desc');
        ...
        $res->each(function (&$item){
                        $item['creator'] = app(Users::class)->where('id', $item['creator_id'])->value('username');
                        $item['user_count'] = $item->users->count('uid');
                        unset($item['users']);
